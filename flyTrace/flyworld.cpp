...
osg::ref_ptr<osg::Geode> FlyWorld::createShapes()
{
	...
	osg::ref_ptr<osg::Image> CSPlusImage = new osg::Image();
	CSPlusImage = osgDB::readImageFile(CSPlusImageFileName);

	if (CSPlusImage.valid())
		imageSequence->addImage(CSPlusImage.get());

	osg::ref_ptr<osg::Image> CSMinusImage = new osg::Image();
	CSMinusImage = osgDB::readImageFile(CSMinusImageFileName);

	if (CSMinusImage.valid())
		imageSequence->addImage(CSMinusImage.get());

	osg::ref_ptr<osg::Image> testImage = new osg::Image();
	testImage = osgDB::readImageFile(testImageFileName);

	if (testImage.valid())
		imageSequence->addImage(testImage.get());
	
	//osg::ref_ptr<osg::Image> trainingImage = new osg::Image();
	//trainingImage = osgDB::readImageFile(trainingImageFileName);

	//if (trainingImage.valid())
		//imageSequence->addImage(trainingImage.get());
...
void FlyWorld::setVisible(bool v)
{
	...
	//	viewer->getCamera()->setClearColor(osg::Vec4(0, 0, 0, 1)); // black background
	//else
	//	viewer->getCamera()->setClearColor(osg::Vec4(1, 1, 1, 1)); // white background
}