...
double viewWidth = 912.0/3.0;
double viewHeight = 1140.0 * 2.0;

float radius = 2.0;		
float height = 3.5;	

double centerDistance = radius + 8.0;
double sideDistance = radius + 8.0;

double centerCull = 0.0;
double sideCull = 0.0;

double camHorLoc = 0;
double camVertLoc = -height/2;
double sidecamVertLoc = -height / 2;
...
const char* displayFile = "displaySettings_fly2p.txt";
...
osg::ref_ptr<osg::Geode> createShapes()
{
	...
	osg::ref_ptr<osg::Cylinder> cyl = new osg::Cylinder(osg::Vec3d(0.0, 0.0, 0.0), radius, height);
...
void run()
{
	...
	//std::cout << reflection;
	//std::cout << leftCombined;
	//std::cout << rightCombined;
	...
	while (!viewer.done())
	{
		...
		viewer.getSlave(1)._viewOffset = leftCombined*osg::Matrixd::lookAt(osg::Vec3d(camHorLoc, sideDistance, sidecamVertLoc), osg::Vec3d(camHorLoc, depth, sidecamVertLoc), osg::Vec3d(0, 0, 1));
		...
		viewer.getSlave(2)._viewOffset = rightCombined*osg::Matrixd::lookAt(osg::Vec3d(camHorLoc, sideDistance, sidecamVertLoc), osg::Vec3d(camHorLoc, depth, sidecamVertLoc), osg::Vec3d(0, 0, 1));
